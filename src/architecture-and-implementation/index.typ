...
If there is a match, the instruction is guaranteed to be a load instruction as the only way the entry would have ended up in the predictor storage in the first place is by .
It can be reasonable to perform a simple check to see whether the instruction is a load and use clock-gating to conserve the power spent accessing the predictor storage.
...
Predictions are not buffered, meaning that if there is available capacity in the LSU the next cycle, but no predictions generated, the LSU stalls.
...
Predictions are prioritised over various wakeup and commit signals, but can not take priority over the `_incoming` signals.
...
We have made efforts to properly synchronise the predictions that leave the LSU with the uOPs that arrive from dispatch/rename2 to the LSU.
Because predictions are based on uOPs that have not yet passed through rename, the LDQ index has not yet been generated.

Most of the time, the naive approach works by assuming a fixed number of cycles.
...
Instead, we compare the instruction addresses of the predictions and the 
